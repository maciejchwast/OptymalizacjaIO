...
Rosen(matrix(*ff)(matrix, matrix, matrix), matrix x0, matrix s0, double alpha, double beta, double epsilon, int Nmax,
    ...
    try {
        solution X(x0), Xt;
        int n = get_dim(X);
        matrix l(n, 1), p(n, 1), s(s0), D = ident_mat(
                n); // l - lambda, p - licznik porazek, s - dlugosci krokow, d - macierz kierunkami poczatkowtmi
        X.fit_fun(ff, ud1, ud2);
        while (true) {
            for (int i = 0; i < n; i++) {
                Xt.x = X.x + s(i) * D[i];
...
solution pen(matrix(*ff)(matrix, matrix, matrix), matrix x0, double c0, double dc, double epsilon, int Nmax, matrix ud1,
             matrix ud2) {
    try {
